// 1. FUNZIONE GLOBALE: così può essere chiamata anche da salvaModifica()
async function caricaOrdiniDalDatabase() {
    const listaUl = document.querySelector('.lista_ordini');
    if (!listaUl) return; // Protezione se la lista non esiste nella pagina
    ...
    try {
        ...
        if (result.success && result.data.length > 0) {
            ...
            for (let i = 0; i < result.data.length; i++) {
                ...
                // Mapping colore (ntc.js deve essere caricato nell'HTML)
        ...
        if (listaUl) listaUl.innerHTML = '<p>Errore tecnico nel caricamento.</p>';
    }
}

// 2. AVVIO AL CARICAMENTO DELLA PAGINA
