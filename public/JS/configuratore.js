...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const imageFiles = {
        ...
        tshirt: {
            front: 'allegati/Fronte-Maglia.png',
            back: 'allegati/Retro-Maglia.png'
        }
    };
    ...
    // Aggiorna pannelli configurazione
    function updateUI() {
        ...
        const activePanel =
            (activeProduct === 'hoodie') ? controlsHoodie :
            (activeProduct === 'shorts') ? controlsShorts :
            controlsTshirt;

        // Mostra campi fronte/retro
    ...
    // Aggiorna taglie disponibili
    function aggiornaTaglieConfiguratore() {

        if (!tagliaSelect) return;

        tagliaSelect.innerHTML =
            '<option value="" selected disabled>Scegli Taglia...</option><option value="UNISEX">UNISEX</option>';

        if (activeProduct === 'hoodie' || activeProduct === 'tshirt') {

            ['XS', 'S', 'M', 'L', 'XL', 'XXL'].forEach(t => {
                tagliaSelect.innerHTML += `<option value="${t}">${t}</option>`;
            });
    ...
    function switchState(newProduct, newView) {
        ...
        // Aggiorna bottoni prodotto
        document.querySelectorAll('.product-selectors button')
            .forEach(btn => btn.classList.remove('active'));

        if (activeProduct === 'hoodie')
            document.getElementById('select-hoodie').classList.add('active');

        if (activeProduct === 'shorts')
            document.getElementById('select-shorts').classList.add('active');

        if (activeProduct === 'tshirt')
            document.getElementById('select-tshirt').classList.add('active');
        ...
        // Ripristina canvas salvato
        canvas.clear();

        if (canvasStates[activeProduct][activeView]) {
            canvas.loadFromJSON(
                canvasStates[activeProduct][activeView],
                () => canvas.renderAll()
            );
        }
    ...
    function setupDynamicText(inputId, fontId, orientId, colorId, customId, defaultTop, defaultLeft, defaultAngle) {
        ...
        function updateText() {
            ...
            let existingText = canvas.getObjects()
                .find(obj => obj.customId === customId);
        ...
        if ((e.key === 'Delete' || e.key === 'Backspace') && e.target.tagName !== 'INPUT') {
            ...
            if (activeObj) {
                ...
                const inputMap = {
                    ...
                    'hoodie-C': 'text-hoodie-front-center',
    ...
    document.getElementById('btn-conferma-salva').addEventListener('click', async () => {
        ...
        if (!tagliaScelta)
            return mostraToast("⚠️ Seleziona una taglia!", "warning");
        ...
        try {
            ...
            fabric.Image.fromURL(imgPath, async function (baseImg) {
                ...
                baseImg.scaleToWidth(canvas.width)
                    .set({ left: 0, top: 0, selectable: false });
                ...
                const res = await fetch(
                    'https://api.cloudinary.com/v1_1/dfjburbax/image/upload',
                    {
                        method: 'POST',
                        body: formData
                    }
                ...
                // Invio dati al carrello principale
                if (data.secure_url && window.parent && window.parent.salvaImmagineConfiguratore) {

                    let tipologiaScelta =
                        (activeProduct === 'hoodie') ? 'Felpa' :
                        (activeProduct === 'shorts') ? 'Pantalone' :
                        'T-shirt';

                    window.parent.salvaImmagineConfiguratore(
                        data.secure_url,
                        tipologiaScelta,
                        currentColor,
                        tagliaScelta,
